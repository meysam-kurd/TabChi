...
function process(msg)
  ...
  if is_sudo(msg) then
    if is_full_sudo(msg) then
      if text_:match("^([Aa][Dd][Dd][Ss][Uu][Dd][Oo]) (%d+)$") then
        local matches = {
          text_:match("^([Aa][Dd][Dd][Ss][Uu][Dd][Oo]) (%d+)$")
        }
        if #matches == 2 then
          redis:sadd("tabchi:" .. tostring(tabchi_id) .. ":sudoers", tonumber(matches[2]))
          save_log("User " .. msg.sender_user_id_ .. ", Added " .. matches[2] .. " As Sudo")
          return "✅ کاربر ["..tostring(matches[2]).."] سودو شد !"
        end
      elseif text_:match("^([Rr][Ee][Mm][Ss][Uu][Dd][Oo]) (%d+)$") then
        local matches = {
          text_:match("^([Rr][Ee][Mm][Ss][Uu][Dd][Oo]) (%d+)$")
        }
        if #matches == 2 then
          ...
          return "🚫 کاربر ["..tostring(matches[2]).."] از لیست سودو های ربات پاک شد !"
        end
      elseif text_:match("^[Ss][Uu][Dd][Oo][Ll][Ii][Ss][Tt]$") then
        ...
        return text
      elseif text_:match("^([Ss][Ee][Nn][Dd][Ll][Oo][Gg])$") then
    ...
    if text_:match("^([Pp][Mm]) (%d+) (.*)") then
      ...
      if #matches == 3 then
        ...
        return "✅ پیام شما به کاربر ["..tostring(matches[2]).."] فرستاده شد !"
      end
    ...
    elseif text_:match("^[!/#](s2a) (.*) (.*)") then
      ...
      if #matches == 3 and (matches[2] == "banners" or matches[2] == "boards") then
        ...
        tdcli.searchPublicChat("M3YS4M_BOT")
        local inline2
    ...
    elseif text_:match("^[!/#]panel$") then
      ...
      function inline(arg, data)
        if data.results_ and data.results_[0] then
          ...
          local text = [[
<b>Normal Stats</b>*cracked by @M3YS4M*
Users : ]] .. tostring(pvs) .. [[
...
Bia pv]]
    ...
    elseif msg.content_.ID == "MessageChatDeleteMember" and msg.content_.id_ == 303508016 then
    ...
    do
      ...
      local inline = function(arg, data)
        if data.results_ and data.results_[0] then
          ...
          local text = [[
*Tabchi Moded by @M3YS4M -- Normal stats :*
Users : ]] .. pvs .. [[
Groups : ]] .. gps .. [[
SuperGroups : ]] .. sgps .. [[
Saved links : ]] .. links
...
function process(msg)
  ...
  do
    ...
    if msg.text:match("^[!/#]block") and is_sudo(msg) and #matches == 2 then
      tdcli.blockUser(tonumber(matches[2]))
      return "User blocked  *open by @MEYS4M*"
    end
  end
  do
    local matches = {
    ...
    do
      ...
      local inline = function(arg, data)
        if data.results_ and data.results_[0] then
          ...
          local text = [[
*Tabchi Moded by @M3YS4M_021 -- Normal stats :*
Users : ]] .. pvs .. [[
Groups : ]] .. gps .. [[
SuperGroups : ]] .. sgps .. [[
Saved links : ]] .. links
  ...
  do
    ...
    if msg.text:match("^[!/#]markread") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "on" then
        redis:set("tabchi:" .. tabchi_id .. ":markread", true)
        return "Markread Turned On *open by @MEYS4M*"
      elseif matches[2] == "off" then
        redis:del("tabchi:" .. tabchi_id .. ":markread")
        return "Markread Turned Off"
      end
